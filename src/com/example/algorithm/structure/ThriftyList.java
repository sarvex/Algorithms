...
public class ThriftyList<T> extends AbstractList<T> implements List<T>,
  ...
  /**
   * Helper function to merge two {@link ListInternal} instances into the target
   * {@link ListInternal} instance. The target capacity should be equal to the
   * sum of the capacities of its constituent lists.
   * 
   * @param l1
   *          the first constituent list.
   * @param l2
   *          the second constituent list.
   * @return the target {@link ListInternal} instance containing the data of l1
   *         followed by the data of l2.
   */
   ...
   * destination lists. The sum of the individual capacities of the destination
   * lists should be equal to the capacity of the source list.
   * 
   * @param alignRight
  ...
  public void add(int index, T item) {
    ...
    if (projectedIndex < smallListCapacity) {
      ...
      sublistOffset = sublistIndex == headSublistIndex ? index : projectedIndex
          & ((1 << smallSublistSizeExp) - 1);
    } else {
  ...
  public Object clone() {
    try {
      ...
      for (int j = 0; j < result.sublists.size(); j++) {
        result.sublists.set(j, (CircularListInternal<T>) result.sublists.get(j)
            .clone());
      }
      return result;
   ...
   * {@link #smallSublistSizeExp} to the beginning. Note that each time capacity
   * is added to this {@link ThriftyList}, small lists will be merged to large
   * lists.
   * 
   ...
   * large sublist if only a single small sublist exists. Do nothing if no small
   * sublists exist.
   */
  protected void mergeNextSmallSublists() {
    if (smallSublistCount >= 2) {
      CircularListInternal<T> small2 = sublists.remove(--smallSublistCount);
      CircularListInternal<T> small1 = sublists.get(--smallSublistCount);
      sublists.set(smallSublistCount,
                   merge(small1,
                         small2,
      ...
      if (size > 0) {
        ...
        tailSublistIndex = sublists.getTail().isEmpty() ? sublists.size() - 2
            : sublists.size() - 1;
      } else {
  ...
  protected T removeImpl(int sublistIndex, int sublistOffset) {
    ...
    if (sublistIndex < headSublistIndex + (calculateSublistsUsed() >>> 1)) {
      ...
      if (sublists.get(tailSublistIndex).isEmpty()) {
        tailSublistIndex = Math.max(headSublistIndex, tailSublistIndex - 1);
        shrinkTail();
      }
    }
    assert checkListState(false, false);
  ...
  /**
   * Shrink the head of this list. Note that each time capacity is removed from
   * this {@link ThriftyList}, large lists will be split to small lists.
   */
  ...
  /**
   * Shrink the tail of this list. Note that each time capacity is removed from
   * this {@link ThriftyList}, large lists will be split to small lists.
   */
   ...
   * responsibilities to {@link ThriftyList} itself.
   */
  protected static class FixedListInternal<T> implements ListInternal<T>,
    ...
    public Object clone() {
      try {
        FixedListInternal<T> result = (FixedListInternal<T>) super.clone();
        result.array = Arrays.copyOf(array, array.length);
        return result;
      } catch (CloneNotSupportedException e) {
        throw new InternalError();
  ...
  /**
   * <p>
   * Interface for internal helper lists to {@link ThriftyList} as instances are
   * not required to perform bounds checking (all such responsibilities are
   * delegated to {@link ThriftyList} itself).
   * 
   * <p>
  ...
  protected static interface ListInternal<T> {
     ...
     *          the target array.
     * @param targetIndex
     *          the array index at which to begin filling the target array with
     *          this list's data.
     * @param index
     *          the index of this list at which to begin filling the target
     *          array.
     * @param count
     ...
     * @return the index of the first element equal to the specified Object, -1
     *         if none.
     */
    public int indexOf(Object o);
     ...
     * @param o
     *          the Object for which to search.
     * @return the index of the last element equal to the specified Object, -1
     *         if none.
     */
    public int lastIndexOf(Object o);
