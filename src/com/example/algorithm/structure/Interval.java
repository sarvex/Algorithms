...
public class Interval<T extends Comparable<T>> implements
   ...
   *          the low endpoint.
   * @param isClosedOnLow
   *          true if this interval contains its low endpoint, false otherwise.
   * @param high
   *          the high endpoint.
   * @param isClosedOnHigh
   *          true if this interval contains its high endpoint, false otherwise.
   */
  ...
  public boolean overlaps(Interval<T> interval) {
    ...
    if (!interval.isClosedOnLow && low.compareTo(interval.low) <= 0
        ...
        && interval.low.compareTo(low) <= 0 && low.compareTo(interval.high) < 0) {
      return true;
    }
    return false;
  }
